...
- **Transcript readability**: Public transcript lines now use explicit tags like `[CHAT][Alice] ...` instead of `Alice: ...`, and transcript content is flattened to one physical line per event. This avoids false speaker changes when players address each other with colons and keeps `tail -f logs/transcript-*.txt` stable.
