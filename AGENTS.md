...
- **Log persistence is append-style**: `src/logger.ts` now writes structured logs as `logs/game-*.jsonl` and appends transcript lines to `logs/transcript-*.txt` one event at a time, so `tail -f` works as a live stream. Replay loading in `src/replay/loadReplay.ts` supports both the new JSONL format and legacy `game-*.json` array snapshots.
- **Transcript readability**: Tail transcripts now emit fully tagged lines like `[PUBLIC][CHAT][Alice] ...`, `[PRIVATE][SYSTEM] ...`, or `[FACTION][FACTION_CHAT][Bob] ...` instead of `Alice: ...`. Transcript content is flattened to one physical line per event, which avoids false speaker changes when players address each other with colons and keeps `tail -f logs/transcript-*.txt` stable.
...
- **Live public-turn failures**: If a public discussion call exhausts retries, the harness now emits a fixed public line (`I hit a response error and cannot answer this turn.`) instead of silently falling back to `SKIP`, so transport/API failures stay visible to the table.
